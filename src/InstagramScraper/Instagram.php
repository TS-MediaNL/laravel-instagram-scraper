...
use TsMedia\LaravelInstagramScraper\InstagramScraper\Exception\InstagramChallengeRecaptchaException;
use TsMedia\LaravelInstagramScraper\InstagramScraper\Exception\InstagramChallengeSubmitPhoneNumberException;
...
use TsMedia\LaravelInstagramScraper\InstagramScraper\Model\Like;
...
use TsMedia\LaravelInstagramScraper\InstagramScraper\Model\Story;
use TsMedia\LaravelInstagramScraper\InstagramScraper\Model\Tag;
use TsMedia\LaravelInstagramScraper\InstagramScraper\Model\Thread;
use TsMedia\LaravelInstagramScraper\InstagramScraper\Model\UserStories;
use TsMedia\LaravelInstagramScraper\InstagramScraper\Model\Highlight;
use TsMedia\LaravelInstagramScraper\InstagramScraper\TwoStepVerification\ConsoleVerification;
use TsMedia\LaravelInstagramScraper\InstagramScraper\TwoStepVerification\TwoStepVerificationInterface;
use InvalidArgumentException;
use TsMedia\LaravelInstagramScraper\InstagramScraper\Http\Request;
...
class Instagram
{
    ...
    protected $userAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36';
    ...
    private static function getErrorBody(mixed $rawError): string
    {
        ...
        if (is_object($rawError)) {
            ...
            foreach ($rawError as $key => $value) {
                $keyStr   = is_scalar($key)   ? (string) $key   : json_encode($key,   JSON_THROW_ON_ERROR);
                $valueStr = is_scalar($value)  ? (string) $value : json_encode($value, JSON_THROW_ON_ERROR);
    ...
    public static function httpCodeToString(int $code): string
    {
        return match ($code) {
            100 => 'Continue',
